...
const CurrencySelector = ({ isDark = false }) => {
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                ...
                className={`flex items-center gap-1 text-sm hover:text-[#0d9488] transition-colors ${isDark ? 'text-white' : ''}`}
            >
                ...
                <div className="absolute top-full right-0 mt-2 bg-white shadow-lg border border-gray-100 min-w-[160px] z-50">
                    {currencies.map(curr => (
                        <button
                            ...
                            className={`w-full text-left px-4 py-2 text-sm flex justify-between items-center hover:bg-[#fafaf9] transition-colors ${currency === curr ? 'text-[#0d9488] font-medium' : 'text-[#1c1917]'
                                }`}
                        >
